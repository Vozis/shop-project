import React, {
  FC,
  MutableRefObject,
  useCallback,
  useContext,
  useRef,
  useState,
} from "react";

import styles from "./Search.module.scss";

import debounce from "lodash.debounce";
import {Simulate} from "react-dom/test-utils";
import input = Simulate.input;
import {useAppDispatch, useAppSelector} from "../../store/store";
import {filterSelector} from "../../store/selectors/selectors";
import {setSearchValue} from "../../store/slices/filterReducer";

interface SearchProps {
  value: string;
}

const Search: FC = () => {
  const dispatch = useAppDispatch();
  const [value, setValue] = useState<string>("");
  const {searchValue} = useAppSelector(filterSelector);
  // const inputRef = useRef() as MutableRefObject<HTMLInputElement>;
  const inputRef = useRef<HTMLInputElement>(null);

  /* const searchDebounce = useCallback(() => {
                           callback;
                         }, [inputRef]);*/

  const onClickClear = (event: React.MouseEvent<SVGSVGElement>) => {
    console.log(event);
    dispatch(setSearchValue(""));
    setValue("");
    inputRef.current?.focus();
  };

  const updateSearchValue = useCallback(
    debounce((value: string) => {
      dispatch(setSearchValue(value));
    }, 1000),
    []
  );

  const onChangeInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    setValue(event.target.value);
    updateSearchValue(event.target.value);
  };

  /* const handlePressInput = ({ code }: React.KeyboardEvent<HTMLElement>) => {
                           if (code === "Enter") {
                             setSearchValue(value);
                             setValue("");
                           }
                         };*/

  return (
    <div className={styles.root}>
      <svg
        className={styles.icon}
        enableBackground="new 0 0 32 32"
        id="EditableLine"
        version="1.1"
        viewBox="0 0 32 32"
        xmlns="http://www.w3.org/2000/svg"
      >
        <circle
          cx="14"
          cy="14"
          fill="none"
          id="XMLID_42_"
          r="9"
          stroke="#000000"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeMiterlimit="10"
          strokeWidth="2"
        ></circle>
        <line
          fill="none"
          id="XMLID_44_"
          stroke="#000000"
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeMiterlimit="10"
          strokeWidth="2"
          x1="27"
          x2="20.366"
          y1="27"
          y2="20.366"
        ></line>
      </svg>
      <input
        ref={inputRef}
        className={styles.input}
        placeholder="Поиск пиццы..."
        value={value}
        onChange={onChangeInput}
        // onKeyDown={handlePressInput}
      />
      {searchValue && (
        <svg
          className={styles.clearIcon}
          onClick={onClickClear}
          viewBox="0 0 20 20"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M10 8.586L2.929 1.515 1.515 2.929 8.586 10l-7.071 7.071 1.414 1.414L10 11.414l7.071 7.071 1.414-1.414L11.414 10l7.071-7.071-1.414-1.414L10 8.586z"></path>
        </svg>
      )}
    </div>
  );
};

export default Search;
